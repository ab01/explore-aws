...
grep "Amway - Security Hardening" /etc/sysctl.conf >/dev/null 2>&1
if [ $? -ne 0 ]; then
	cat >> /etc/sysctl.conf << EOF

### Amway - Security Hardening
...
# Configure postfix
if [ x"`grep \#\#\#\ Amway\ \-\ Custom\ Postfix /etc/postfix/main.cf`" == x ]; then
cat >> /etc/postfix/main.cf << EOF

### Amway - Custom Postfix
myorigin = amway.com
