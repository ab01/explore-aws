...
#export AmwayLocation1="10.0.0.0/8"
#export AmwayLocation2="172.16.0.0/12"
#export PAPublicIP1="52.69.124.223"
...
UpdateRoutes() {
 ...
 for Location in $AmwayLocation1 $AmwayLocation2;do
...
UpdateIpsec() {
 ...
 leftsubnet=$VpcCIDR
 right=$PAPublicIP1
 rightsubnets={$AmwayLocation1,$AmwayLocation2}
 auto=start" > /etc/ipsec.d/Amway_Tunnel1.conf
 echo "$OpenSwanEIP $PAPublicIP1: PSK \"$PATunnelPssword1\"" > /etc/ipsec.d/Amway_Tunnel1.secrets
 echo "conn Amway_Tunnel1
 dpddelay=5
 dpdtimeout=5
 dpdaction=restart
 ...
 right=$PAPublicIP2
 rightsubnets={$AmwayLocation1,$AmwayLocation2}
 auto=start" > /etc/ipsec.d/Amway_Tunnel2.conf.DISABLED
 
 echo "$OpenSwanEIP $PAPublicIP2: PSK \"$PATunnelPssword2\"" > /etc/ipsec.d/Amway_Tunnel2.secrets.DISABLED
...
chmod 555 /etc/init.d/ShutdownEth0
