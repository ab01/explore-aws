...
if [ $LifecycleState == "Pending:Wait" ]; then
	...
	#cp /usr/share/tomcat/logs/catalina.out /tmp/catalina.out-$(date +%F_%R)
	#/usr/bin/aws s3 cp s3://asg-lifecyclehook-test/Artifacts/sample.war /usr/share/tomcat/webapps/
	mkdir -p /opt/test
	...
	aws autoscaling complete-lifecycle-action --lifecycle-action-result CONTINUE --instance-id $INSTANCE_ID --lifecycle-hook-name PendingHook --auto-scaling-group-name $AutoScalingGroupName
